# PEP 0014: Add Monitor Redis Type

Extend the `monitor` package in @monitor to support the monitor type "redis".

Follow the existing patterns for other monitor types (e.g. HTTP), in particular:

- Embed the `Base` struct.
- Add tests to the client it self (@monitor_test.go) as well as to the `monitor` package in @monitor/monitor_redis_test.go.
