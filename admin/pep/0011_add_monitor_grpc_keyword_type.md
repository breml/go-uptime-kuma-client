# PEP 0011: Add Monitor gRPC Keyword Type

Extend the `monitor` package in @monitor to support the monitor type "grpc_keyword".

Follow the existing patterns for other monitor types (e.g. HTTP), in particular:

- Embed the `Base` struct.
- Add tests to the client it self (@monitor_test.go) as well as to the `monitor` package in @monitor/monitor_grpc_keyword_test.go.
