# PEP 0013: Add Monitor PostgreSQL Type

Extend the `monitor` package in @monitor to support the monitor type "postgres".

Follow the existing patterns for other monitor types (e.g. HTTP), in particular:

- Embed the `Base` struct.
- Add tests to the client it self (@monitor_test.go) as well as to the `monitor` package in @monitor/monitor_postgres_test.go.
