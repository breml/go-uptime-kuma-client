...
// open-wa (https://github.com/open-wa/wa-automate-nodejs) is a self-hosted
// WhatsApp gateway. The server posts the alert to
// "<apiUrl>/api/sessions/<session>/messages/send-text", once per chat ID.
type OpenWa struct {
	Base
	OpenWaDetails
}
...
type OpenWaDetails struct {
	...
	// The server strips trailing slashes before it appends the endpoint, a lone
	// "/" is kept. The URL is stored verbatim, a trailing slash survives the
	// round trip.
	...
	// The server trims each entry and drops empty ones, the notification fails
	// if no valid chat ID remains.
